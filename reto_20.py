...
def distribur(pesos:list):
  ...
  for i in range(len(pesos)):
    ...
    for j in range(len(pesos[i])):
      ...
      if pesos[i][j]:
        columnas.append(pesos[i][j])
        divisor += 1
        
      
      resultado = sum(columnas) / divisor
